...
import io.ocepgen.modules.sys.dao.SysSchoolDao;
import io.ocepgen.modules.sys.dao.SysSchoolGradeDao;
import io.ocepgen.modules.sys.dao.SysSchoolSemesterDao;
...
import io.ocepgen.modules.sys.service.SysSchoolGradeService;
...
public class SysSchoolServiceImpl extends CrudServiceImpl<SysSchoolDao, SysSchoolEntity, SysSchoolDTO> implements SysSchoolService {
    ...
    public void delete(Long[] ids) {
        for (Long schoolId : ids) {
            ...
            if (sysSchoolGradeDao.selectOne(qw1) != null ) {
                throw new OcepgenException("学校下有年级，无法删除，请先删除年级信息");
            }
            if (sysSchoolSemesterDao.selectOne(qw2) != null ) {
                throw new OcepgenException("学校下有学期信息，无法删除，请先删除学期信息");
            }
        }
        super.delete(ids);
    }
    ...
    public String getSchoolNameById(Long schoolId) {
        LambdaQueryWrapper<SysSchoolEntity> qw = new LambdaQueryWrapper<>();

        qw.eq(SysSchoolEntity::getSchoolId, schoolId);

        return sysSchoolDao.selectOne(qw).getSchoolName();
    }
}
