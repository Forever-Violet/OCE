<template>
  <div class="mod-job__schedule">
    <el-form :inline="true" :model="state.dataForm" @keyup.enter="state.getDataList()">
      <el-form-item>
        <el-input v-model="state.dataForm.beanName" placeholder="bean名称" clearable>
          <template #prefix>
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="318">
        <template v-slot="scope">
          <el-button v-if="state.hasPermission('sys:schedule:update')" type="success" size="small"  @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          
                  
          <el-button v-if="state.hasPermission('sys:schedule:pause')" type="warning" size="small" @click="pauseHandle(scope.row.id)">暂停</el-button>
          <el-button v-if="state.hasPermission('sys:schedule:resume')" type="info" size="small" @click="resumeHandle(scope.row.id)">恢复</el-button>
          <el-button v-if="state.hasPermission('sys:schedule:run')" type="primary" size="small" @click="runHandle(scope.row.id)">执行</el-button>
          <el-button v-if="state.hasPermission('sys:schedule:delete')" type="danger" size="small" @click="state.deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
