<template>
  <div class="mod-sys__menu">
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" show-overflow-tooltip row-key="id" border style="width: 100%">
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="195">
        <template v-slot="scope">
          <el-button v-if="state.hasPermission('sys:menu:save') && scope.row.menuType === 0" type="primary" size="small" @click="addHandle(scope.row)">新增</el-button>
          
          <el-button v-if="state.hasPermission('sys:menu:update')" type="success" size="small"  @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          
          
          <el-button v-if="state.hasPermission('sys:menu:delete')" type="danger" size="small" @click="state.deleteHandle(scope.row.id)">删除</el-button>
