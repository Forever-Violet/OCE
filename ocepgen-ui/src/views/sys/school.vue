<template>
  <div class="mod-sys__school">
    <el-form :inline="true" :model="state.dataForm" @keyup.enter="state.getDataList()">
      <el-form-item>
        <el-input v-model="state.dataForm.schoolName" placeholder="学校名称" clearable @change="state.getDataList()">
          <template #prefix>
      ...
      <el-form-item>
        <ren-select v-model="state.dataForm.unitType" dict-type="unitType" placeholder="单位类型" @change="state.getDataList()"></ren-select>
      </el-form-item>
      ...
      <el-form-item>
        <el-button v-if="state.hasPermission('sys:school:export')" type="info" @click="state.exportHandle()">导出</el-button>
      </el-form-item>
    </el-form>
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
          ...
          <el-button v-if="state.hasPermission('sys:school:update')" type="success" size="small"  @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          
          
          <el-button v-if="state.hasPermission('sys:school:delete')" type="danger" size="small" @click="state.deleteHandle(scope.row.id)">删除</el-button>
