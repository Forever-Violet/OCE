<template>
  <el-drawer v-model="visible" :title="!dataForm.id ? '新增用户' : '修改用户'" size="50%" :close-on-press-escape="false" :close-on-click-modal="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      <el-form-item prop="username" label="用户名">
        <el-input v-model="dataForm.username" placeholder="用户名"></el-input>
      </el-form-item>
      <el-form-item v-if="hasSchoolListPermission" prop="schoolName" label="所属学校">
        <el-select v-model="dataForm.schoolId" filterable placeholder="选择学校" clearable @change="resetClassList"> <!--单选 去掉multiple , clearable重置-->
      ...
      <el-form-item prop="roleIdList" label="角色配置" class="role-list">  <!--多选-->
        <el-select v-model="dataForm.roleIdList" filterable multiple placeholder="角色配置">
          <el-option v-for="role in roleList" :key="role.id" :label="role.name" :value="role.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item prop="classIdList" label="班级配置" class="role-list">
        <el-select v-model="dataForm.classIdList" filterable multiple placeholder="班级配置">
