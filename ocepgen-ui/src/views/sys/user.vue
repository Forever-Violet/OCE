  ...
  <div class="mod-sys__user">
    <el-form :inline="true" :model="state.dataForm" @keyup.enter="state.getDataList()">
      <el-form-item v-if="hasSchoolListPermission">
        <el-select v-model="state.dataForm.schoolId" placeholder="选择学校" clearable @change="state.getDataList()"> 
          <el-option v-for="school in schoolList" :key="school.schoolId" :label="school.schoolName" :value="school.schoolId"></el-option>
        </el-select>
      </el-form-item>

      <el-form-item>
        <el-input v-model="state.dataForm.username" placeholder="用户名" clearable @change="state.getDataList()">
          <template #prefix>
      ...
      <el-form-item>
        <ren-select v-model="state.dataForm.gender" dict-type="gender" placeholder="性别" @change="state.getDataList()"></ren-select>
      </el-form-item>

      <el-form-item>
        <el-select v-model="state.dataForm.roleId" placeholder="选择角色" clearable @change="state.getDataList()">
    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" border @selection-change="state.dataListSelectionChangeHandle" @sort-change="state.dataListSortChangeHandle" style="width: 100%">
      ...
      <el-table-column prop="mobile" label="手机号" show-overflow-tooltip sortable="custom" header-align="center" align="center"></el-table-column>
      ...
      <el-table-column prop="roleNameList" label="角色" show-overflow-tooltip header-align="center" align="center">
        <template v-slot="scope">
          <el-tag v-for="roleName in scope.row.roleNameList" size="small" :type="roleName=='老师'?'primary':'success'">{{ roleName }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="classNameList" label="班级" show-overflow-tooltip header-align="center" align="center" width="200">
        <template v-slot="scope">
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
            ...
            <el-button v-if="state.hasPermission('sys:user:update')" type="success" size="small"  @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          
          
            <el-button v-if="state.hasPermission('sys:user:delete')" type="danger" size="small" @click="state.deleteHandle(scope.row.id)">删除</el-button>
...
const getRoleList = async () => {
  const res = await baseService.get("/sys/role/list");
